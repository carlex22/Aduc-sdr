# ltx_manager_helpers.py (Corrigido para `progress` opcional)
...
class LtxWorker:
    ...
    def __init__(self, device_id='cuda:0'):
        ...
        config_file_path = os.path.join(config_dir, "ltxv-13b-0.9.8-distilled.yaml")
        
        # Se o arquivo de config não existir, podemos criá-lo com um padrão
        if not os.path.exists(config_file_path):
             print(f"Arquivo de configuração não encontrado em {config_file_path}, criando um padrão.")
             default_config = {
...
class LtxPoolManager:
    ...
    def __init__(self, device_ids=['cuda:0', 'cuda:1']):
        ...
        # Lida com o caso de não haver GPUs disponíveis
        if not torch.cuda.is_available() or not device_ids:
            print("Nenhuma GPU CUDA encontrada ou especificada. Operando em modo CPU.")
            self.workers = [LtxWorker('cpu')]
        else:
            self.workers = [LtxWorker(device_id) for device_id in device_ids if torch.cuda.is_available()]
            if not self.workers:
                print("Nenhuma das GPUs especificadas estava disponível. Operando em modo CPU.")
                self.workers = [LtxWorker('cpu')]
    ...
    def generate_video_fragment(
        ...
        current_fragment_index: int, output_path: str, progress=None # Alterado para aceitar None
    ):
        worker_to_use = None
        try:
            with self.lock:
            ...
            kwargs = {
                ...
                "enhance_prompt": False, "num_inference_steps": int(num_inference_steps)
            }
            
            # <<< A CORREÇÃO ESTÁ AQUI >>>
            # Só atualiza a barra de progresso SE ela foi fornecida (não é None).
            if progress:
            ...
            # Garante que o recorte não exceda as dimensões do tensor
            if slice_h is not None and abs(slice_h) > result_tensor.shape[3]: slice_h = None
            if slice_w is not None and abs(slice_w) > result_tensor.shape[4]: slice_w = None
...
# Ajuste para ser mais flexível com a configuração de GPU
# Se a variável de ambiente CUDA_VISIBLE_DEVICES estiver definida, use-a.
if "CUDA_VISIBLE_DEVICES" in os.environ:
    available_devices = [f"cuda:{i}" for i in range(len(os.environ["CUDA_VISIBLE_DEVICES"].split(',')))]
else:
    available_devices = [f"cuda:{i}" for i in range(torch.cuda.device_count())]

if not available_devices:
    ltx_manager_singleton = LtxPoolManager(device_ids=[])
else:
    ltx_manager_singleton = LtxPoolManager(device_ids=available_devices if len(available_devices) > 1 else [available_devices[0], available_devices[0]])