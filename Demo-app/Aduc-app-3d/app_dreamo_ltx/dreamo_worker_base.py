# dreamo_worker_base.py (GPU-A: cuda:0)
# Worker para gerar os keyframes base em baixa resolução.
...
class Generator:
    def __init__(self, device_id='cuda:0'):
        ...
        print(f"WORKER PINTOR-BASE: Usando dispositivo: {self.gpu_device}")

        print(f"WORKER PINTOR-BASE: Carregando modelos DreamO para a CPU...")
        ...
        print("WORKER PINTOR-BASE: Modelos prontos (na CPU).")

    def to_gpu(self):
        if self.gpu_device.type == 'cpu': return
        print(f"WORKER PINTOR-BASE: Movendo modelos para {self.gpu_device}...")
        ...
        print(f"WORKER PINTOR-BASE: Modelos na GPU {self.gpu_device}.")

    def to_cpu(self):
        if self.gpu_device.type == 'cpu': return
        print(f"WORKER PINTOR-BASE: Descarregando modelos da GPU {self.gpu_device}...")
        ...
        print(f"WORKER PINTOR-BASE: Modelos descarregados.")
...
print("Inicializando o Worker Pintor-Base (GPU-A)...")
hf_token = os.getenv('HF_TOKEN')
if hf_token: huggingface_hub.login(token=hf_token)
dreamo_base_singleton = Generator(device_id='cuda:0')
print("Worker Pintor-Base pronto.")