...
# --- app_cpu.py (NOVINHO-6.0-CPU: Versão para HF Spaces gratuitos) ---
# AVISO: A geração de imagens e vídeos será extremamente lenta.
...
pipeline_instance = create_ltx_video_pipeline(
    ...
    device='cpu' # Força a pipeline a ser carregada na CPU
)
...
def run_keyframe_generation(storyboard, ref_images_tasks, progress=gr.Progress()):
    ...
    try:
        # Lógica de GPU removida, DreamO já está na CPU.
        ...
        for i, scene_description in enumerate(storyboard):
            progress(i / len(storyboard), desc=f"[CPU Lenta] Pintando Keyframe {i+1}/{len(storyboard)}")
            ...
            # Esta chamada agora usará a CPU
...
def run_video_production(
    ...
    seed = int(time.time())
    try:
        # Lógica de GPU removida, a pipeline já está na CPU
        ...
        for i in range(num_transitions):
            ...
            progress(i / num_transitions, desc=f"[CPU Lenta] Filmando Fragmento {fragment_num}...")
...
def run_ltx_animation(
    ...
    progress(0, desc=f"[CPU Lenta] Filmando Cena {current_fragment_index}...");
    output_path = os.path.join(WORKSPACE_DIR, f"fragment_{current_fragment_index}_full.mp4")
    target_device = 'cpu'
    try:
        # Lógica de GPU removida
        conditioning_items = [ConditioningItem(load_conditioning_tensor(p, height, width), s, t) for p, s, t in conditioning_items_data]
        ...
        pass # Nenhum recurso de GPU para liberar
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    gr.Markdown("# NOVIM-6.0-CPU (Painel de Controle do Diretor)\n*By Carlex & Gemini & DreamO*")
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            with gr.Accordion("Controles Avançados de Timing e Performance", open=False):
                ...
                video_fps_slider = gr.Slider(label="FPS do Vídeo", minimum=12, maximum=30, value=24, step=1)
                num_inference_steps_slider = gr.Slider(label="Etapas de Inferência", minimum=10, maximum=50, value=25, step=1)
                slicing_checkbox = gr.Checkbox(label="Usar Attention Slicing (Ignorado na CPU)", value=True, interactive=False)
