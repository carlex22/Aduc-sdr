...
# (Cabeçalho e importações permanecem os mesmos...)

# --- app.py (NOVINHO-4.9: Lógica de Transição Inicial Corrigida) ---
...
# ... (robust_json_parser, extract_image_exif, run_storyboard_generation - permanecem iguais) ...
...
# ... (get_dreamo_prompt_for_transition, run_keyframe_generation - permanecem iguais) ...
...
def run_keyframe_generation(storyboard, initial_ref_image_path, sequential_ref_task, progress=gr.Progress()):
    ...
    try:
        ...
        for i, scene_description in enumerate(storyboard):
            ...
            log_history += f"  - Roteiro: '{scene_description}'\n  - Usando {len(recent_references_paths)} referências visuais: {[os.path.basename(p) for p in recent_references_paths]}\n  - Prompt do D.A.: \"{dreamo_prompt}\"\n"
...
# <<< NOVA FUNÇÃO para o primeiro fragmento (sem "Caminho") >>>
def get_initial_motion_prompt(user_prompt: str, start_image_path: str, destination_image_path: str, dest_scene_desc: str):
    ...
    try:
        ...
        prompt_file = "prompts/initial_motion_prompt.txt" # Novo prompt
        with open(os.path.join(os.path.dirname(__file__), prompt_file), "r", encoding="utf-8") as f: template = f.read()
        ...
        raise gr.Error(f"O Cineasta de IA (Inicial) falhou: {e}. Resposta: {getattr(e, 'text', 'No text available.')}")
...
def get_dynamic_motion_prompt(user_prompt: str, story_history: str, memory_image_path: str, path_image_path: str, destination_image_path: str, path_scene_desc: str, dest_scene_desc: str):
    ...
    try:
        ...
        raise gr.Error(f"O Cineasta de IA (Dinâmico) falhou: {e}. Resposta: {getattr(e, 'text', 'No text available.')}")
...
def run_video_production(prompt_geral, keyframe_images_state, scene_storyboard, seed, cfg, cut_frames_value, progress=gr.Progress()):
    ...
    try:
        ...
        for i in range(num_transitions):
            ...
            if i == 0:
                # Caso especial para o PRIMEIRO fragmento (Início -> Destino)
                start_path = keyframe_images_state[0]
                destination_path = keyframe_images_state[1]
                dest_scene_desc = scene_storyboard[0]
                
                log_history += f"  - Início: {os.path.basename(start_path)}\n  - Destino: {os.path.basename(destination_path)}\n"
                ...
                # Lógica normal para os fragmentos subsequentes (Memória -> Caminho -> Destino)
                memory_path, path_path, destination_path = kinetic_memory_path, keyframe_images_state[i], keyframe_images_state[i+1]
                path_scene_desc = scene_storyboard[i-1] # Ajuste de índice
...
# ... (process_image_to_square, load_conditioning_tensor, etc. - permanecem iguais) ...
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    gr.Markdown("# NOVINHO-4.9 (Lógica de Transição Inicial Corrigida)\n*By Carlex & Gemini & DreamO*")
    ...
    gr.Markdown(
        ...
        Nossa arquitetura é inspirada no conceito de "Handoff" da engenharia, como em uma corrida de revezamento. Cada fragmento de vídeo passa o "bastão" para o próximo de forma fluida.
        *   **O Bastão (`Eco`):** O último frame de um fragmento cortado (enquanto ainda está em movimento) carrega a "energia cinética" da cena.
        *   **O Handoff (A Geração):** O próximo fragmento começa a partir deste `Eco` dinâmico, herdando a "física" do movimento e da iluminação.
        *   **A Sincronização (Cineasta de IA):** Para cada Handoff, o Cineasta de IA (`Γ`) analisa o (`Eco`), o (`Keyframe` anterior) e o (`Keyframe` futuro) para criar uma instrução de movimento precisa.
        """
    )
