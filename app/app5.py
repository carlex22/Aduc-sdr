...
# --- app.py (NOVINHO-5.1: Lógica "Big Bang" Final) ---
...
def run_video_production(prompt_geral, keyframe_images_state, scene_storyboard, seed, cfg, cut_frames_value, progress=gr.Progress()):
    if not keyframe_images_state or len(keyframe_images_state) < 3: 
        raise gr.Error("Pinte pelo menos 2 keyframes (para um total de 3+ imagens incluindo a inicial) para produzir uma transição.")
    ...
    try:
        ...
        kinetic_memory_path = None # Começa nulo, será criado após o primeiro fragmento
        with Image.open(keyframe_images_state[1]) as img: width, height = img.size # Largura e altura são baseadas no primeiro keyframe real
        ...
        for i in range(num_transitions):
            ...
            if i == 0:
                # O "Big Bang": transição de K_1 para K_2
                start_path = keyframe_images_state[1]      # K_1
                destination_path = keyframe_images_state[2] # K_2
                dest_scene_desc = scene_storyboard[1]     # Ato 2
                ...
                # Lógica de Handoff Cinético: Eco_(i-1) -> K_i -> K_(i+1)
                memory_path = kinetic_memory_path
                path_path = keyframe_images_state[i+1]      # K_2, K_3, ...
                destination_path = keyframe_images_state[i+2] # K_3, K_4, ...
                
                path_scene_desc = scene_storyboard[i]
                dest_scene_desc = scene_storyboard[i+1]
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    gr.Markdown("# NOVINHO-5.1 (Lógica 'Big Bang' Final)\n*By Carlex & Gemini & DreamO*")
