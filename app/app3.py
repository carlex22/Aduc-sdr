...
def run_keyframe_generation(storyboard, initial_ref_image_path, sequential_ref_task, progress=gr.Progress()):
    ...
    try:
        ...
        for i, scene_description in enumerate(storyboard):
            ...
            # A lógica para gerar o prompt de texto continua a mesma, baseada na imagem mais recente.
            dreamo_prompt = get_dreamo_prompt_for_transition(current_ref_image_path, scene_description)
            
            # --- INÍCIO DA NOVA LÓGICA ---
            # Pega as últimas 3 imagens (ou quantas estiverem disponíveis) como referência visual.
            recent_references_paths = keyframe_paths[-3:]
            log_history += f"  - Roteiro: '{scene_description}'\n"
            log_history += f"  - Usando {len(recent_references_paths)} referências visuais: {[os.path.basename(p) for p in recent_references_paths]}\n"
            log_history += f"  - Prompt do D.A. (baseado em {os.path.basename(current_ref_image_path)}): \"{dreamo_prompt}\"\n"
            yield {keyframe_log_output: gr.update(value=log_history), keyframe_gallery_output: gr.update(value=keyframe_paths)}

            # Constrói a lista de referências para o DreamO.
            reference_items = []
            for ref_path in recent_references_paths:
                reference_items.append({
                    'image_np': np.array(Image.open(ref_path).convert("RGB")), 
                    'task': sequential_ref_task # Usa a tarefa 'ip' (ou outra selecionada) para todas as referências.
                })
            # --- FIM DA NOVA LÓGICA ---

            output_path = os.path.join(WORKSPACE_DIR, f"keyframe_{i+1}.png")
            # Envia a lista de referências para o gerador.
            ...
            current_ref_image_path = output_path # Atualiza a referência "mais recente" para a próxima iteração.
...
def run_video_production(prompt_geral, keyframe_images_state, scene_storyboard, seed, cfg, cut_frames_value, progress=gr.Progress()):
    ...
    try:
        ...
        for i in range(num_transitions):
            ...
            if i == 0:
                conditioning_items_data = [(memory_path, int(0), 1.0), (destination_path, int(END_COND_FRAME), 1.0)]
            else:
